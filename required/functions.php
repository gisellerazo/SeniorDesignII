...
if (ini_set('session.use_only_cookies', 1) == FALSE) { // check if cookies can be used
...
$secure, $httpsonly); // keep same lifetime, path, and domain
...
// Function to login
function login($username, $password, $mysqli) { // $mysqli is the db connection
...
$stmt->fetch();
if ($stmt->num_rows == 1) { // only one row is expected
...
} else {return false;} // bad password
} else {return false;} // number of rows != 1
} else {return false;} // mysqli statement error
}
// Function to check if user is logged in
function login_check($mysqli) { // $mysqli is the db connection
if (isset($_SESSION['user_id'], $_SESSION['username'], $_SESSION['login_string'])) { // check if each of
...
if (hash_equals($login_check, $login_string)) { // session is not hijacked if logins match
return true; // if logins match, then user is logged in
} else {return false;} // login check failed
} else {return false;} // number of rows != 1
} else {return false;} // mysqli statement error
} else {return false;} // isset error?
}
?>